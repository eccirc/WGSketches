...
var main = (p) => {
  ...
  p.preload = () => {
    ...
    networkImage = p.loadImage('logo_clean_background.png')
  }
  ...
  p.draw = () => {
    ...
    particlesArray.forEach(element => {
      if (element.thisInfoBannerOn) {
        drawInfoBanner(element.index, 20, 20)
      }
    })

    // text displayer ---------
    p.fill(241, 184, 88, 240)
    p.rect((canvasWidth * 0.5 - 192) * transformer, 65 * transformer, 300 * transformer, (canvasHeight - 160) * transformer, 30 * transformer)
    ...
    p.text("subscribe if you wanna", (canvasWidth * 0.5 - 42) * transformer, (canvasHeight * 0.5 - 45) * transformer)
    p.text("be part of We-Guild!", (canvasWidth * 0.5 - 42) * transformer, (canvasHeight * 0.5 - 5) * transformer)

    p.tint(255)
    p.image(networkImage,
      (canvasWidth * 0.5 - 67) * transformer,
      180 * transformer,
      50 * transformer,
      50 * transformer)
    // ------------------------ 
